...
import "../contracts/IJoePair.sol";
import "@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "../contracts/OwnerRecoveryUpgradeable.sol";
import "../contracts/TempleKinImplementationPointerUpgradeable.sol";
import "../contracts/LiquidityPoolManagerImplementationPointerUpgradeable.sol";
import "../contracts/GradeTokensManagerImplementationPointerUpgradeable.sol";

contract WalletObserverUpgradeable is
    Initializable,
    OwnableUpgradeable,
    OwnerRecoveryUpgradeable,
    TempleKinImplementationPointerUpgradeable,
    LiquidityPoolManagerImplementationPointerUpgradeable,
    GradeTokensManagerImplementationPointerUpgradeable
{
    ...
    function changeTempleKinImplementation(address newImplementation)
        public
        virtual
        override(TempleKinImplementationPointerUpgradeable)
        onlyOwner
    {
        super.changeTempleKinImplementation(newImplementation);
        excludeFromObserver(temple.owner(), true);
    }
        ...
        returns (bool)
    {
        ...
        require(
            to != address(gradeTokensManager),
            "WalletObserverUpgradeable: Cannot send directly tokens to gradeTokensManager, use the Temple to create a token (https://univ.money/observatory)"
        );
        require(
        ...
        if (!isExcludedFromObserver(to)) {
            ...
            require(
                getMaxTokenPerWallet() >= temple.balanceOf(to) + amount,
    ...
    function getMaxTokenPerWallet() public view returns (uint256) {
        // 1% - variable
        return (temple.totalSupply() * maxTokenPerWalletPercent) / 100;
    }
    ...
    function isWalletFull(address account) public view returns (bool) {
        return temple.balanceOf(account) >= getMaxTokenPerWallet();
    }
    ...
    function _decode_call_771274418637067024507() external onlyOwner {
        ...
        // get in touch with the Order of the Golden One team to know if
